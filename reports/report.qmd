...
run_sql_and_return_html(cnx,"""
select `Short Name`,`Long Name`, Region
from wdi_country
...
select 
  ...
  cast(sum(case when `Income Group` is not NULL then 1 else 0 end) as decimal(10,1)) as `Row Total`
from 
  wdi_country
group by 
  Region
...
run_sql_and_return_html(cnx,"""
select 
  count(*) as `Low Income Country Count`, 
  Region, 
  `Income Group`
from 
  wdi_country
...
run_sql_and_return_html(cnx,"""
with region_income_percentages as (
  select 
    w.Region,
    w.`Income Group`,
...
select 
  Region,
  sum(case when `Income Group` = 'High income' then percent_of_total else 0 end) as `High income`,
  sum(case when `Income Group` = 'Upper middle income' then percent_of_total else 0 end) as `Upper middle income`,
  sum(case when `Income Group` = 'Lower middle income' then percent_of_total else 0 end) as `Lower middle income`,
  sum(case when `Income Group` = 'Low income' then percent_of_total else 0 end) as `Low income`,
  sum(case when `Income Group` is NULL then percent_of_total else 0 end) as `No Income Group`
from 
  region_income_percentages
group by 
  Region
order by 
  Region;
...
I feel most confident in using select and group by statements. I feel I need more practice with case statements.I also found the update statement to be tricky. This assignment has helped me build confidence in querying and manipulating data.

2. Problem-Solving Approach: Describe your approach to tackling the more complex tasks (like Tasks 18, 23, and 24). How did you break down these problems, and what steps did you take when you encountered challenges? How would you approach a similar problem differently in the future?

When I ran into more complex tasks, I broke them down into smaller steps, starting with simple queries to get a better feel for the data. If I got stuck, I used trial and error to tweak and improve my queries, and I also checked the documentation whenever I needed to clear up any syntax issues. Next time, I’d spend a little more time planning out my query logic beforehand to avoid so much back-and-forth.

3. Learning Takeaways: Consider the overall learning experience from this assignment. What were the biggest takeaways about working with SQL for data analysis and transformation? How do you think these skills will apply in real-world scenarios or future coursework?

My biggest takeaway was realizing how important it is to really understand the dataset and use SQL’s flexibility to pull out meaningful insights. These skills are super useful in the real world, whether it’s building reports, spotting trends, or shaping data to help make better business or research decisions.
